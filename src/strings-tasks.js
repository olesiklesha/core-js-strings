...
function sumOfCodes(str) {
  ...
  return str
    .split('')
    .reduce((acc, el, index) => acc + str.charCodeAt(index), 0);
}
...
function isPalindrome(str) {
  ...
  return strToCompare === str.toLowerCase().replace(pattern, '');
}
...
function invertCase(str) {
  return str
    .split('')
    .map((el) =>
      el.localeCompare(el.toUpperCase()) === -1
        ? el.toUpperCase()
        : el.toLowerCase()
    )
    .join('');
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (code >= 65 && code <= 90) {
      const cur = code + 13;
      const newCode = cur <= 90 ? cur : 65 + cur - 91;
      res += String.fromCharCode(newCode);
    } else if (code >= 97 && code <= 122) {
      const cur = code + 13;
      const newCode = cur <= 122 ? cur : 97 + cur - 123;
      res += String.fromCharCode(newCode);
    } else {
      res += str[i];
    }
